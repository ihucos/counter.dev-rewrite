...
6. The dashboard SPA reads three JSON endpoints (behind the API hostname `api.counter.dev` locally `api.counterdev.test`; the SPA resolves it at runtime and the backend's `CorsMiddleware` grants the SPA origins): `GET /me` (session bootstrap: user record, share token, demo/sessionless flags), `GET /sites` (a readonly DRF API and the source of truth for which sites an account has) and `GET /query` (per-site `Sum(total)` by category/item for an arbitrary open-ended `start`/`end` range, plus the recent-visits log from the Redis zset). Fixed presets (today, last 7 days, …) are computed client-side as `start`/`end` values. The dashboard fetches only on page load and on user interactions (date-range or site changes) — no live updates. Guests read the same endpoints via `?user=<uuid>&token=<token>`.

## Gotchas

- Postgres is fed solely by sync, which deletes keys as it drains them — if sync is down past the TTL, today's data is lost.
- `/query` reads Postgres, so ingest lags the dashboard by up to ~1 s (plus sync's poll interval); the dashboard only picks up new data on reload or interaction.

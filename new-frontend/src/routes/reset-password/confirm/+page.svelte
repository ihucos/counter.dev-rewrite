<script>
  import { page } from '$app/state';
  import { api } from '$lib/api.js';

  let uid = $state('');
  let token = $state('');
  let pwd1 = $state('');
  let pwd2 = $state('');
  let error = $state('');
  let loading = $state(false);
  let done = $state(false);

  // Read query params from the URL
  $effect(() => {
    const params = page.url.searchParams;
    uid = params.get('uid') || '';
    token = params.get('token') || '';
  });

  async function handleConfirm() {
    error = '';
    if (!pwd1 || pwd1.length < 6) { error = 'Password must be at least 6 characters.'; return; }
    if (pwd1 !== pwd2) { error = 'Passwords do not match.'; return; }
    loading = true;
    try {
      await api.confirmPasswordReset({ uid, token, new_password1: pwd1, new_password2: pwd2 });
      done = true;
    } catch (e) {
      error = e.message || 'Failed to reset password.';
    } finally {
      loading = false;
    }
  }
</script>

<div class="page">
  <div class="card">
    <div class="logo"></div>
    <h1>Set New Password</h1>

    {#if done}
      <div class="success">Password reset successfully!</div>
      <a href="/" class="btn-primary" style="display:block;text-align:center;">Sign In</a>
    {:else if !uid || !token}
      <div class="error">Invalid or missing reset link.</div>
      <a href="/" class="btn-primary" style="display:block;text-align:center;">Back to Sign In</a>
    {:else}
      <form onsubmit={(e) => { e.preventDefault(); handleConfirm(); }}>
        {#if error}<div class="error">{error}</div>{/if}
        <label>
          New Password
          <input type="password" bind:value={pwd1} placeholder="Min. 6 characters" required disabled={loading} />
        </label>
        <label>
          Confirm Password
          <input type="password" bind:value={pwd2} placeholder="Repeat password" required disabled={loading} />
        </label>
        <button type="submit" class="btn-primary" disabled={loading || !pwd1 || !pwd2}>
          {loading ? 'Resetting...' : 'Reset Password'}
        </button>
      </form>
    {/if}
  </div>
</div>

<style>
  .page {
    display: flex; justify-content: center; align-items: center; min-height: 100vh;
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  }
  .card {
    background: white; border-radius: 16px; padding: 40px; width: 100%;
    max-width: 400px; box-shadow: 0 10px 40px rgba(0,0,0,0.2);
  }
  .logo {
    width: 48px; height: 48px; background: linear-gradient(135deg, #667eea, #764ba2);
    border-radius: 12px; margin-bottom: 24px;
  }
  h1 { margin: 0 0 12px; font-size: 24px; color: #1a1a2e; }
  label { display: block; margin-bottom: 16px; font-size: 14px; color: #666; }
  input {
    display: block; width: 100%; padding: 10px 12px; margin-top: 6px;
    border: 1px solid #ddd; border-radius: 8px; font-size: 16px;
  }
  input:focus { outline: none; border-color: #4361ee; box-shadow: 0 0 0 3px rgba(67,97,238,0.15); }
  .btn-primary {
    background: linear-gradient(135deg, #667eea, #764ba2); color: white; border: none;
    padding: 12px 24px; border-radius: 8px; font-size: 16px; cursor: pointer;
    width: 100%; margin-top: 8px; font-weight: 600;
  }
  .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
  .error {
    background: #fee; color: #c0392b; padding: 12px; border-radius: 8px;
    margin-bottom: 16px; font-size: 14px;
  }
  .success {
    background: #d4edda; color: #155724; padding: 16px; border-radius: 8px;
    margin-bottom: 24px; font-size: 14px;
  }
</style>

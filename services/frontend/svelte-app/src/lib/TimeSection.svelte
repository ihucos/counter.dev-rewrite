<script>
  ...
  // Normalized hour data with all 24 hours present
  let normalizedHours = $derived.by(() => {
    const pad = Object.fromEntries([...Array(24).keys()].map(i => [i, 0]));
    const data = { ...pad };
  ...
  let maxHourCount = $derived(
    Math.max(...hourEntries.map(e => e.count), 1)
  );

  // Normalized weekday data
  let normalizedWeekdays = $derived.by(() => {
    const pad = Object.fromEntries(WEEKDAY_LABELS.map((label, idx) => [idx, 0]));
    const data = { ...pad };
  ...
  let maxWeekdayCount = $derived(
    Math.max(...weekdayEntries.map(e => e.count), 1)
  );

  // Daytime breakdown (for time-of-day view)
  let dayParts = $derived.by(() => {
    const morning = { label: 'Morning (6–12)', hours: [6, 7, 8, 9, 10, 11], count: 0 };
    const afternoon = { label: 'Afternoon (12–18)', hours: [12, 13, 14, 15, 16, 17], count: 0 };
    const evening = { label: 'Evening (18–24)', hours: [18, 19, 20, 21, 22, 23], count: 0 };
    const night = { label: 'Night (0–6)', hours: [0, 1, 2, 3, 4, 5], count: 0 };
  ...
  let maxDayPartCount = $derived(
    Math.max(...dayParts.map(d => d.count), 1)
  );

  function hasHourData() {
    return Object.keys(hourData || {}).length > 0;
  }
  function hasWeekdayData() {
    return Object.keys(weekdayData || {}).length > 0;
  }
  function hasData() {
    return hasHourData() || hasWeekdayData();
  }
</script>
...
{#if hasData()}
  <div class="time-section-panel">
    ...
    <!-- Day tab: time-of-day breakdown (morning, afternoon, evening, night) -->
    {#if activeTab === 'day'}
          ...
          {#each dayParts as part}
            <div class="part-row">
              ...
              <div class="part-bar-track">
                <div
                  class="part-bar-fill"
                  style="width: {(part.count / maxDayPartCount) * 100}%"
                ></div>
              </div>
              <div class="part-value">{numberFormat(part.count)}</div>
              <div class="part-pct">
                {maxDayPartCount > 0 ? Math.round((part.count / maxDayPartCount) * 100) : 0}%
              </div>
            </div>
          {/each}
        </div>
    ...
    <!-- Hour tab: 24-hour breakdown -->
    {:else if activeTab === 'hour'}
      <div class="tab-content">
        <div class="hour-chart">
          {#each hourEntries as entry}
            <div class="hour-bar-wrapper" title="{entry.label}: {numberFormat(entry.count)} visits">
              <div
                class="hour-bar"
                style="height: {Math.max((entry.count / maxHourCount) * 100, 2)}px"
                class:zero={entry.count === 0}
              ></div>
              <span class="hour-label">
                {#if entry.hour === 0}
                  12a
                {:else if entry.hour === 12}
                  12n
                {:else if entry.hour < 12}
                  {entry.hour}a
                {:else}
    ...
    <!-- Week tab: day-of-week breakdown -->
    {:else if activeTab === 'week'}
      <div class="tab-content">
        <div class="weekday-chart">
          {#each weekdayEntries as entry}
            <div class="weekday-bar-wrapper" title="{entry.label}: {numberFormat(entry.count)} visits">
              <div
                class="weekday-bar"
                style="height: {Math.max((entry.count / maxWeekdayCount) * 100, 2)}px"
                class:zero={entry.count === 0}
              ></div>
